...
def generate_and_test_audio(text, lang_code, file_name):
    ...
    try:
        ...
        if lang_code == 'bn':
            ...
            print(f"  🗣️ Using male-oriented voice settings for Bangla")
        else:
...
def process_questions():
    print("🔍 Fetching questions with missing audio URLs...")
    print(f"🧪 TEST MODE: {TEST_MODE} (audio will be {'played locally' if TEST_MODE else 'uploaded to Supabase'})\n")
    
    # Fetch questions where any audio URL is missing (NULL or empty string)
    # and the question is linked to a subtopic.
    response = execute_with_retry(
        lambda: supabase.table('questions').select(
            'id, subtopic_id, text_it, text_en, text_bn, audio_it_url, audio_en_url, audio_bn_url'
        )\
            .or_(
                'audio_it_url.is.null,audio_it_url.eq."",audio_en_url.is.null,audio_en_url.eq."",audio_bn_url.is.null,audio_bn_url.eq.""'
            )\
            .not_.is_('subtopic_id', 'null')\
            .limit(BATCH_SIZE).execute(),
        operation_desc="fetch questions batch"
    )
    ...
    for q in questions:
        if q.get('subtopic_id') is None:
            print(f"⚠️ Skipping Question ID {q.get('id')} because subtopic_id is null")
            continue
        ...
        print(f"\n📝 Text Preview:")
        print(f"  IT: {safe_preview(q.get('text_it'))}...")
        print(f"  EN: {safe_preview(q.get('text_en'))}...")
        print(f"  BN: {safe_preview(q.get('text_bn'))}...")
        print()
        ...
        if is_missing_audio_url(q.get('audio_it_url')):
        ...
        if is_missing_audio_url(q.get('audio_en_url')):
        ...
        if is_missing_audio_url(q.get('audio_bn_url')):
        ...
        if not TEST_MODE:
            ...
            if update_data:
                execute_with_retry(
                    lambda: supabase.table('questions').update(update_data).eq('id', q_id).execute(),
                    operation_desc=f"update question {q_id}"
                )
...
if __name__ == "__main__":
    try:
        if PROCESS_ALL:
            ...
            print("🚀 BATCH MODE: Processing ALL questions with missing audio URLs")
            ...
            while True:
                ...
                response_before = execute_with_retry(
                    lambda: supabase.table('questions')\
                        ...
                        .limit(1)\
                        .execute(),
                    operation_desc="count remaining questions"
                )
